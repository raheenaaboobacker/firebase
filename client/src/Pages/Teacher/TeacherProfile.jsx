  ...
  useEffect(() => {
    const id=localStorage.getItem("id")
     ...
     const unsub = onSnapshot(doc(db, "user", id), (doc) => {
       ...
       setName(CryptoJS.AES.decrypt(data.name, process.env.REACT_APP_SECRET_KEY).toString(CryptoJS.enc.Utf8))
       setAddress(CryptoJS.AES.decrypt(data.address, process.env.REACT_APP_SECRET_KEY).toString(CryptoJS.enc.Utf8))
       setPhone(CryptoJS.AES.decrypt(data.phone, process.env.REACT_APP_SECRET_KEY).toString(CryptoJS.enc.Utf8))

       setRole(data.role)
       setDept(data.dept)
       setPlace(CryptoJS.AES.decrypt(data.place, process.env.REACT_APP_SECRET_KEY).toString(CryptoJS.enc.Utf8))
       console.log(name);
